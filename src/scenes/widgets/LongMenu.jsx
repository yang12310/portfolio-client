  ...
  const handleOk = () => {
    if(command == "삭제"){
      const postDelete = async () => {
        const response = await fetch(`http://13.124.74.251:3500/posts/delete/${userId}/${postId}`, {
          method: "DELETE",
      ...
      const postPatch = async () => {
        const response = await fetch(`http://13.124.74.251:3500/posts/patch/${userId}/${postId}`, {
          method: "PATCH",
          headers: {
        ...
        : <WidgetWrapper sx={{border:"none",marginBottom:"0", padding:"0em 1em"}}>
            ...
            <Box>
              <img
                ...
                src={`http://13.124.74.251:3500/assets/${picturePath}`}
              />
            </Box>
          </WidgetWrapper>
      }
